                ...
                if not in_comment_block:
                    if ignore_till_eol:
                        ...
                        if c == '"' or c == "'":
                            ...
                            errors.append(
                                "  ERROR: Found a tab on line {}.".format(line_number))
                        elif c in ["(", "[", "{"]:
                            brackets.append(c)
                        elif c == ")":
                            if not brackets or brackets[-1] in ["[", "{"]:
                                errors.append(
                                    "  ERROR: Missing parenthesis '(' on line {}.".format(line_number))
                            brackets.append(c)
                        elif c == "]":
                            if not brackets or brackets[-1] in ["(", "{"]:
                                errors.append(
                                    "  ERROR: Missing square bracket '[' on line {}.".format(line_number))
                            brackets.append(c)
                        elif c == "}":
                            ...
                            if not brackets or brackets[-1] in ["(", "["]:
                                errors.append(
                                    "  ERROR: Missing curly brace '{{' on line {}.".format(line_number))
                            brackets.append(c)
                        ...
                        if check_for_semicolon:
                            ...
                            if c not in [" ", "\t", "\n", "/"]:
                                ...
                                if c not in ["]", ")", "}", ";", ",", "&", "!", "|", "="] and not valid_keyword_after_code(content, char_index):
                                    errors.append(
                                        "  ERROR: Possible missing semicolon ';' on line {}.".format(line_number))
    ...
    for filepath in sqf_files:
        ...
        if errors:
            print("\nFound {} error(s) in {}:".format(
                len(errors), os.path.relpath(filepath, root_dir)))
